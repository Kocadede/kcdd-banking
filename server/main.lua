...
AddEventHandler("vlast-banking-addBankHistory", function(data)
    xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer ~= nil then
        MySQL.Async.execute('INSERT INTO bankhistory (identifier, header, amount, msg, type) VALUES (@identifier, @header, @amount, @msg, @type)', {
...
ESX.RegisterServerCallback("kocadede-getPlayerAccountHistory", function(source, cb)
    ...
    if xPlayer ~= nil then
        MySQL.Async.fetchAll("SELECT * FROM bankhistory WHERE identifier = @identifier", {
            ["@identifier"] = xPlayer.identifier
        }, function(data)
